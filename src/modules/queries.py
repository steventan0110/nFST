...
class EncodedQuery(QueryBase):
    ...
    @abstractmethod
    def __actually_encode__(
        self, to_encode: torch.Tensor, mask: torch.Tensor, **kwargs
    ):
        pass
    ...
    def query(self, state: torch.Tensor, **kwargs):
        ...
        if encoded_shape[0] == state_shape[0]:
            ...
            assert (
                encoded_shape[0] == 1
            ), "shapes: encoded_shape {} state_shape {}".format(
                encoded_shape, state_shape
            )
            ...
            return self.__actually_query__(
                state, expanded_encoded, expanded_mask, **kwargs
            )

    @abstractmethod
    def __actually_query__(
        self, state: torch.Tensor, encoded: torch.Tensor, mask: torch.Tensor, **kwargs
    ):
        pass
...
class BiGRUEncodedQuery(EncodedQuery):
    def __init__(
        ...
        self.attn = BilinearAttention(
            vector_dim=state_hid_dim, matrix_dim=2 * hid_dim, normalize=True
        )
    ...
    def __actually_encode__(
        ...
        """
        if self.previously_encoded:
            embedded = self.activation(
                self.embedder(to_encode)
...
class QueryPi(EncodedQuery):
    ...
    def update_buffer_indices(
        self, to_update: torch.Tensor, buffer: torch.Tensor, indices: torch.Tensor
    ):
        try:
            buffer[torch.arange(to_update.shape[0]), indices].masked_scatter(
                (to_update != self.pad), to_update
            )
    ...
    def update_indices(self, indices, to_update):
        offset = torch.where(
            (to_update != self.pad),
            torch.ones_like(to_update, dtype=torch.int),
    ...
    def query(
        ...
        update_output: Optional[torch.Tensor] = None,
        ...
        self.input_buffer, self.input_indices = self.update_buffer_indices(
            update_input, self.input_buffer, self.input_indices
        )
        self.output_buffer, self.output_indices = self.update_buffer_indices(
            update_output, self.output_buffer, self.output_indices
        )
        input_weighted = self.query_buffer(
            self.input_buffer, state, self.input_gru, self.input_attn
        )
        output_weighted = self.query_buffer(
            self.output_buffer, state, self.output_gru, self.output_attn
        )
    ...
    def init_buffer_indices(self, batch_size, to_encode):
        buffer = torch.empty(
            batch_size, self.max_length, dtype=torch.long, device=to_encode.device
        )
    ...
    def __init__(
        ...
        if self.previously_encoded:
            ...
            if embedder is not None:
                ...
                self.embedder = Embedding(vocab_size, hid_dim)
        self.input_gru = PytorchSeq2SeqWrapper(
            torch.nn.GRU(
                input_size=hid_dim,
                hidden_size=hid_dim,
                num_layers=num_layers,
        ...
        self.output_gru = PytorchSeq2SeqWrapper(
            torch.nn.GRU(
                input_size=hid_dim,
                hidden_size=hid_dim,
                num_layers=num_layers,
        ...
        self.input_attn = BilinearAttention(
            vector_dim=state_hid_dim, matrix_dim=2 * hid_dim, normalize=True
        )
        self.output_attn = BilinearAttention(
            vector_dim=state_hid_dim, matrix_dim=2 * hid_dim, normalize=True
        )
        self.pad = pad
        self.bos = bos
...
class QueryPiGivenXAndY(QueryPi):
    def encode(self, to_encode, mask, to_encode_2=None, mask_2=None, **kwargs):
        ...
        self.output_buffer = self.output_gru(
            inputs=self.embedder(to_encode_2), mask=mask_2
        )
        self.output_mask = mask_2
        self.input_indices = torch.zeros(
            batch_size, dtype=torch.long, device=to_encode.device
        )
        self.output_indices = torch.zeros(
            batch_size, dtype=torch.long, device=to_encode_2.device
        )
    ...
    def query(
        ...
        input_weighted = self.get_weighted(
            self.input_attn, self.input_mask, self.input_buffer, state
        )
        output_weighted = self.get_weighted(
            self.output_attn, self.output_mask, self.output_buffer, state
        )
        return torch.cat([input_weighted, output_weighted], dim=1)


class QueryPiGivenXAndYTransformer(EncodedQuery):
    def __actually_encode__(
        self, to_encode: torch.Tensor, mask: torch.Tensor, **kwargs
    ):
        pass
    ...
    def get_weighted(self, attention, mask, prefix_queried, state):
        ...
        # weights = attention(vector=state, matrix=prefix_queried, matrix_mask=mask)
        weights = torch.einsum("bsh,bh->bs", prefix_queried, state)
        attention_mask = (1 - mask).bool()
        weights.masked_fill_(attention_mask, -1e8)
        weights = self.softmax(weights)
        weighted = weighted_sum(prefix_queried, weights)
        if self.step % 100000 == 0:
            print(weights[0, :])
            print(self.seg_mask[0, :])
        self.step += 1
        return weighted
    ...
    def encode(self, to_encode, mask, to_encode_2=None, mask_2=None, **kwargs):
        seq_len = to_encode.shape[1]
        con_encode = torch.cat((to_encode, to_encode_2), dim=1)
        pad_mask = (~(con_encode == self.pad)).long()
        seg_mask = con_encode.new_full(con_encode.shape, 1)
        seg_mask[:, :seq_len] = 0

        # bz x (seq1 + seq2) x hid_dim
        self.input_buffer = self.model(
            con_encode, attention_mask=pad_mask, token_type_ids=seg_mask
        ).last_hidden_state
    ...
    def query(
        ...
        return self.get_weighted(
            self.attention, self.input_mask, self.input_buffer, state
        )
    ...
    def __init__(
        ...
        super(QueryPiGivenXAndYTransformer, self).__init__()
        self.config = BertConfig(
            vocab_size,
            hidden_size=hid_dim,
            num_hidden_layers=num_layers,
            num_attention_heads=num_heads,
            max_position_embeddings=2 * (max_length + 1),  # concate 2 sentence
            pad_token_id=pad,
            eos_token_id=eos,
            bos_token_id=bos,
        ...
        self.attention = BilinearAttention(
            vector_dim=state_hid_dim, matrix_dim=hid_dim, normalize=True
        )
...
class QueryPiGivenX(BiGRUEncodedQuery):
    def __init__(
        ...
        super(QueryPiGivenX, self).__init__(
            hid_dim,
            num_layers,
            state_hid_dim,
            embedder,
    ...
    def __actually_encode__(
        ...
        input_indices = torch.ones(
            batch_size, dtype=torch.long, device=to_encode.device
        )
        self.register_buffer("input_indices", input_indices)
        to_encode_bos = torch.empty(
            (to_encode.shape[0], 1), dtype=to_encode.dtype, device=to_encode.device
        )
        to_encode_bos.fill_(self.bos)
        to_encode_concatenated = torch.cat([to_encode_bos, to_encode], dim=1)
        monotonic_input_encoded = self.input_gru(
            inputs=self.embedder(to_encode_concatenated), mask=mask
        )
        self.register_buffer(
            "monotonic_input_encoded", monotonic_input_encoded, persistent=False
        )
    ...
    def update_output_buffer(self, to_update: torch.Tensor):
        try:
            self.output_buffer[
                torch.arange(to_update.shape[0]), self.indices
            ].masked_scatter((to_update != self.pad), to_update)
            offset = torch.where(
                (to_update != self.pad),
                torch.ones_like(to_update, dtype=torch.int),
                torch.zeros_like(to_update, dtype=torch.int),
            ...
            self.output_buffer[torch.arange(to_update.shape[0]), self.indices].fill_(
                self.eos
            )
        except Exception as e:
            raise e
    ...
    def __actually_query__(
        ...
        output_prefix_queried = self.output_gru(
            inputs=self.embedder(self.output_buffer), mask=output_mask
        )
        # output_weighted = output_prefix_queried[:, -1, :]
        output_weights = self.output_attn(
            vector=state, matrix=output_prefix_queried, matrix_mask=output_mask
        )
        output_weighted = weighted_sum(output_prefix_queried, output_weights)
        monotonic_input = self.monotonic_input_encoded[
            torch.arange(state.shape[0]), self.input_indices
        ]
